...
import { blankItems } from './constants';

interface Props {
  mapper: MapperPreview;
}

export const MapperPreviewBlock = ({ mapper }: Props) => {
  const {
    serviceName,
    previewName,
    configs,
  ...
  return (
    <div className="p-5 border-2 border-stroke-gray-dark border-dashed  rounded-[10px] flex flex-col gap-[25px]">
      ...
      <div className="flex flex-col gap-[25px] max-w-[650px]">
        <div className="flex flex-col gap-5">
          <FormField clickable label="Name">
            <Input defaultValue={previewName} />
          </FormField>
          <FormField clickable label="Description">
            <TextArea className="w-full" defaultValue={previewDescription} />
          </FormField>
        </div>
